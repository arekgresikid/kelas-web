# KelasWeb - AI-Powered Web Development Platform

Web edukasi modern yang dirancang untuk membantu siapa saja belajar membuat website dari nol dengan bantuan kecerdasan buatan (AI).

![Built with Pollinations](https://img.shields.io/badge/Built%20with-Pollinations-8a2be2?style=for-the-badge&logo=data:image/png;base64,&logoColor=white&labelColor=6a0dad)

## 🚀 Fitur Utama
- **AI Assistant**: Dibangun menggunakan [Pollinations AI](https://pollinations.ai) untuk membantu menjawab pertanyaan teknis siswa secara instan.
- **Project-Based Learning**: Belajar sambil membangun produk nyata seperti Toko Online, Portfolio, dan Tool AI.
- **Modular Markdown**: Materi edukasi berbasis Markdown yang dinamis dan mudah dikelola.
- **Responsive & Modern UI**: Tampilan premium yang optimal di perangkat mobile maupun desktop.
- **Dark Mode Support**: Kenyamanan visual maksimal untuk sesi belajar malam hari.

## 🛠 Tech Stack
- **AI Engine**: [Pollinations.ai](https://pollinations.ai) (Text Generation API)
- **Framework**: React 18 & Vite
- **Styling**: Tailwind CSS
- **Markdown**: React-Markdown & RemarkGFM
- **Animations**: Framer Motion
- **Icons**: Lucide React

## 🤖 AI Integration
KelasWeb menggunakan API dari **Pollinations.ai** untuk mentenagai fitur asisten belajar virtual. Fitur ini memungkinkan siswa untuk:
1. Mendapatkan bantuan koding langsung saat mengerjakan proyek.
2. Bertanya tentang konsep web development dalam bahasa Indonesia.
3. Mendapatkan referensi kode yang akurat dan terkini.

## 📦 Instalasi Lokal

1. Clone repository:
   ```bash
   git clone https://github.com/arekgresikid/kelas-web.git
   ```
2. Instal dependensi:
   ```bash
   npm install
   ```
3. Set .env (Dapatkan API Key di [pollinations.ai](https://pollinations.ai)):
   ```env
   VITE_POLLINATIONS_API_KEY=your_key_here
   ```
4. Jalankan aplikasi:
   ```bash
   npm run dev
   ```

## 🌍 Deploy
Aplikasi ini dioptimalkan untuk deployment statis di **Vercel**, **Cloudflare Pages**, atau **Netlify**.

---
Dibuat dengan ❤️ oleh Kelas Web Indonesia. Didukung oleh teknologi masa depan dari [Pollinations.ai](https://pollinations.ai).
